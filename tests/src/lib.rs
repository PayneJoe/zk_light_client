...
mod test {
    ...
    let exponent = (normalized_target.len() + 1) as u8;
    ...
    fn test_target_bits() {
        ...
        let new_target_difficulty = old_target_difficulty.checked_mul(
            &U256::from_u32(last_epoch_end_block.header.time - last_epoch_begin_block.header.time)).unwrap().checked_div(&U256::from_u32(EXPECTED_EPOCH_SECONDS)).unwrap();
        ...
        println!("0x{:08X}", new_epoch_begin_block.header.bits);
    }
